...
export function createBuild(tenantId: string, config: BuildConfig): string {
  ...
  exec('npm run build', { cwd: process.cwd(), env: buildEnv(widget), timeout: 300000 }, (err) => {
    if (err) {
...
export function deleteBuild(tenantId: string, buildId: string): boolean {
  ...
  fs.rmSync(buildDir, { recursive: true, force: true }); // delete build directory
  return true;
}
...
export function deleteAllTenantBuilds(tenantId: string): boolean {
  ...
  fs.rmSync(tenantDir, { recursive: true, force: true }); // delete all builds for tenant
  return true;
}
